...
public static int creertweet(String strTweet,int idcompte){
	...
	if (veriftweet(strTweet,idcompte)==-1){
		...
		try{
			...
			st.executeUpdate();
			maConnexion.close();
			DAO.getLog().envoyerlog(10, "DAOTweet", "creertweet:Rï¿½ussite insertion enregistrement BDD avec id :text tweet:"+ idcompte +":"+strTweet);
			return 1;
	...
	{
		DAO.getLog().envoyerlog(8, "DAOTweet", "creertweet: tweet :"+strTweet +"et id:"+idcompte+" dï¿½jï¿½ utilisï¿½");
		return -1;
	}
}
...
public static int veriftweet(String strtweet,int idcompte){
    ...
    if(rs.next())
    {
    	 ...
    	 DAO.getLog().envoyerlog(10, "DAOTweet", "veriftweet:Rï¿½ussite rï¿½cuparation  enregistrement  bdd avec  compteid = "+idcompte +":"+strtweet );
    	 maConnexion.close();
    	 return id;           	 
    }
    else
...
/**
 * permet de recupï¿½rer un tweetid avec un tweet
 * @param strtweet
...
public static int IdBytweet(String strtweet){
    ...
    if(rs.next())
    {
    	 ...
    	 DAO.getLog().envoyerlog(10, "DAOTweet", "IdBytweet:Lecture enregistrement rï¿½ussi connexion bdd avec avec tweet = "+strtweet );
    	 maConnexion.close();
    	 return id;           	 
    }
    else
...
public static int ajouternbretweet(String Strtweet){
	try{
        ...
        st.executeUpdate();
        maConnexion.close();
        DAO.getLog().envoyerlog(10, "DAOTweet", "ajouternbretweet: Insertion rï¿½ussie enregistrement pour tweet = "+Strtweet );
		return 1;
	}
	catch(SQLException e)
	{
...
public static int ajouternbretweet(int idtweet){
	try{
        ...
        st.executeUpdate();
        maConnexion.close();
        DAO.getLog().envoyerlog(10, "DAOTweet", "ajouternbretweet: Insertion rï¿½ussie enregistrement pour idtweet = "+idtweet );
		return 1;
	}
	catch(SQLException e)
	{
